...
return function()
    describe("Body", function()
        ...
        test("Set/Get Transform()", function()
            ...
            assert_equal_v3(transform.p, vmath.vector3(0, 0, 0))
            assert_equal_float(transform.q, 0)

            body:SetTransform(vmath.vector3(10, 10, 0), math.pi / 2)
            transform = body:GetTransform()

            assert_equal_v3(transform.p, vmath.vector3(10, 10, 0))
            assert_equal_float(transform.q, math.pi / 2)
        ...
        test("Set Transform no angle", function()
            ...
            assert_equal_v3(transform.p, vmath.vector3(4, 4, 0))
            assert_equal_float(transform.q, math.pi / 2)

            body:SetTransform(vmath.vector3(6, 6, 0), nil)
            transform = body:GetTransform()

            assert_equal_v3(transform.p, vmath.vector3(6, 6, 0))
            assert_equal_float(transform.q, math.pi / 2)
        ...
        test("GetPosition()", function()
            ...
            body:SetTransform(vmath.vector3(5, 10, 0), math.pi / 2)
            assert_equal_v3(body:GetPosition(), vmath.vector3(5, 10, 0))
            w:Destroy()
        end)

        test("GetPositionRaw()", function()
            local w = box2d.NewWorld()
            local body = create_body(w)
            local x,y = body:GetPositionRaw()
            assert_equal(x,0)
            assert_equal(y,0)
            body:SetTransform(vmath.vector3(5, 10, 0), math.pi / 2)
            x,y = body:GetPositionRaw()
            assert_equal(x,5)
            assert_equal(y,10)
            w:Destroy()
        end)
